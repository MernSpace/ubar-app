...
import {DeleteCarouselRequest, ReadCarouselRequest} from "../../../APIRequest/carouselApiRequest.js";
import {useSelector} from "react-redux";
import {DeleteNoticeRequest, ReadNoticeRequest} from "../../../APIRequest/noticeApiRequest.js";
...
const DisplayNotice = () => {
    ...
    const DeleteItem = async (id) => {
        ...
        if(Result.isConfirmed){
            let DeleteResult= await DeleteNoticeRequest(id)
            if(DeleteResult){
                await ReadNoticeRequest();
            }
        }
                                            ...
                                            <div className="table-responsive table-section">
                                                <table className="table ">
                                                    ...
                                                    <tr>
                                                        ...
                                                        <td className="text-uppercase text-secondary text-md font-weight-bolder opacity-9">Notice</td>
                                                    ...
                                                    <tbody>
                                                    {
                                                        DataList.map((item,i)=>
                                                            <tr key={i}>
                                                                ...
                                                                <td className='text-wrap'><p>{item.notice}</p></td>
                                                                <td><p>{new Date(item.updatedAt).toLocaleDateString("en-GB")}</p></td>
