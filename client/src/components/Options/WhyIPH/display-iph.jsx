...
import {DeleteWhyIPHRequest, ReadWhyIPHRequest} from "../../../APIRequest/WhyIPHApiRequest.js";

const DisplayIph = () => {

    useEffect(() => {
        (async () => {
            await ReadWhyIPHRequest()
        })();
    }, []);

    let DataList=useSelector((state)=>(state.whyIPH.WhyIPHList));

    const DeleteItem = async (id) => {
        let Result = await DeleteAlert();
        if(Result.isConfirmed){
            let DeleteResult= await DeleteWhyIPHRequest(id)
            if(DeleteResult){
                await ReadWhyIPHRequest();
            }
        }
